"""Authorized B0-only replay using the unchanged candidate training loop."""
...
def preflight(root):
    ...
    reference=root.parent/'stage2_structure_20260918_v1'
    append=root.parent/'stage2_s1r2r3_append_20260919_v1'
    for local in (root/'source').rglob('*'):
        if local.is_file() and '__pycache__' not in local.parts:
            rel=local.relative_to(root)
            if sha(local)!=sha(reference/rel) or sha(local)!=sha(append/rel):
                raise RuntimeError('Candidate source mismatch: '+str(rel))
    expected_env=read(reference/'runs/R1/seed200/environment.json')
    actual_env=dict(python=sys.version,torch=torch.__version__,cuda=torch.version.cuda,
                    cudnn=torch.backends.cudnn.version())
    if any(actual_env[k]!=expected_env[k] for k in actual_env):
        raise RuntimeError('Training environment differs from completed candidates')
    torch.backends.cudnn.allow_tf32=expected_env['cudnn_allow_tf32']
    torch.backends.cuda.matmul.allow_tf32=expected_env['matmul_allow_tf32']
    allowed,hashed,counts=data_inventory(root)
    install_guard(allowed)
    hashes={p:sha(p) for p in sorted(hashed)}
    old=read(reference/'PREFLIGHT.json')['input_sha256']
    if any(hashes.get(p.casefold())!=h for p,h in old.items()):
        raise RuntimeError('Train input changed since completed Stage1')
    if hashes!=old or hashes!=read(append/'PREFLIGHT.json')['input_sha256']:
        raise RuntimeError('Inputs are not exactly the candidate training inputs')
    weights=root.parent/'weights/PIDNet_S_ImageNet.pth.tar'
    if sha(weights)!='F96E2C96B1ACA1400A6F54AC41093D98B4817F6008A5D869ABE6248551A5F359':
        raise RuntimeError('ImageNet initialization file changed')
    ...
    result=dict(protocol=PROTOCOL_ID,status='PASS',counts=counts,input_sha256=hashes,
                ...
                identical_candidate_source=True,pretrained_sha256=sha(weights),
                environment=actual_env,cudnn_allow_tf32=torch.backends.cudnn.allow_tf32,
...
def run_training(root,arm,seed,resume):
    ...
    for prior in ('stage2_structure_20260918_v1/runs/R1','stage2_s1r2r3_append_20260919_v1/runs/R2',
                  'stage2_s1r2r3_append_20260919_v1/runs/R3'):
        if config!=read(root.parent/prior/f'seed{seed}'/'resolved_config.json'):
            raise RuntimeError('Effective config differs from paired candidate')
    ...
    # R1 is an exact deepcopy without structural changes; no ThermalLoader for B0.
    model=apply_arm(baseline,'R1',seed=seed)
    ...
    del baseline
    if sum(p.numel() for p in model.parameters())!=7717095:
        raise RuntimeError('Baseline parameter count changed')
    model=model.cuda()
