...
class AdmissionTests(unittest.TestCase):
    ...
    def test_unicode_process_path_is_supported(self):
        observation=self.sample(processes={4:r'D:\????\UnknownGpuClient.exe'},
    ...
    def test_remote_desktop_dwm_overhead_is_training_only(self):
        observation=self.sample(processes={4:r'C:\dwm.exe'},
                                engines=[dict(pid=4,percent=2.5)],utilization=5)
        self.assertFalse(blockers(observation,own_pid=9,timing=False))
        self.assertTrue(blockers(observation,own_pid=9,timing=True))

    def test_system_copy_overhead_is_training_only(self):
        observation=self.sample(processes={4:r'C:\System.exe'},
                                engines=[dict(pid=4,percent=.1)],utilization=5)
        self.assertFalse(blockers(observation,own_pid=9,timing=False))
        self.assertTrue(blockers(observation,own_pid=9,timing=True))
    ...
    def test_confirmed_idle_sunlogin_does_not_block_training(self):
        observation=self.sample(processes={4:r'C:\SunloginClient.exe'},
                                engines=[dict(pid=4,percent=35)],utilization=5)
        self.assertFalse(blockers(observation,own_pid=9,timing=False))
        self.assertTrue(blockers(observation,own_pid=9,timing=True))

    def test_heavy_sunlogin_still_blocks_training(self):
        observation=self.sample(processes={4:r'C:\SunloginClient.exe'},
                                engines=[dict(pid=4,percent=41)],utilization=5)
        self.assertTrue(blockers(observation,own_pid=9,timing=False))

    def test_overall_busy_blocks_training(self):
        self.assertTrue(blockers(self.sample(utilization=9),own_pid=9))
