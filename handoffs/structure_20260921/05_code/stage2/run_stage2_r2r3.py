"""R2/R3 stage2 worker after revised Gate 5 and two engineering epochs pass."""
...
from r2r3_protocol import (ARMS,SEEDS,CONFIG_REL,TRAIN_REL,VAL_REL,ANNOTATION_REL,PROTOCOL_ID,
...
def preflight(root):
    ...
    old=read(root/'reference_evidence/TRAIN_INPUT_SHA256_BEFORE.json')
    if any(hashes.get(p.casefold())!=h for p,h in old.items()):
        raise RuntimeError('Train input changed since completed Stage1')
    cpu=read(root/'revised_evidence/cpu/SUMMARY.json')
    gates=read(root/'revised_evidence/cuda/SUMMARY.json')
    cpu_pass={r['arm'] for r in cpu['results'] if r['status']=='REVISED_GATE5_PASS'}
    cuda_pass={r['arm'] for r in gates['results'] if r['status']=='REVISED_GATE5_PASS'}
    engineering_pass=set()
    for arm in ARMS:
        path=root/'engineering_smoke'/arm/'RESULT.json'
        if path.exists():
            record=read(path)
            if record['status']=='TWO_EPOCH_ENGINEERING_PASS' and record['completed_epochs']==2:
                engineering_pass.add(arm)
    eligible=sorted(cpu_pass & cuda_pass & engineering_pass)
    if not eligible:
        raise RuntimeError('No R2/R3 arm passed CPU+CUDA Gate 5 and two engineering epochs')
    ...
    result=dict(protocol=PROTOCOL_ID,status='PASS',counts=counts,input_sha256=hashes,
                ...
                revised_gate5_cpu_and_cuda_passed=True,two_epoch_engineering_passed=True,
                final_amp_head_threshold=1e-2,eligible_arms=eligible)
...
def run_training(root,arm,seed,resume):
    ...
    if arm not in pref['eligible_arms']:
        raise RuntimeError('Arm did not pass all revised engineering prerequisites')
    source=root/'source'
    ...
    for epoch in range(start_epoch,30):
        ...
        wrapped=ThermalLoader(train_loader,seed,epoch) if arm=='R3' else train_loader
